...
#define Echo GPIO_Pin_1		//HC-SR04模块的Echo脚接GPIOA1
#define Trig GPIO_Pin_0		//HC-SR04模块的Trig脚接GPIOA0
...
{ 
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);	//启用GPIOA的外设时钟						
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;		//设置GPIO口为推挽输出
	GPIO_InitStructure.GPIO_Pin = Trig;						//设置GPIO口0
...
//	GPIO_InitStructure.GPIO_Pin = Echo;						//设置GPIO口1
//	GPIO_Init(GPIOA,&GPIO_InitStructure);					//初始化GPIOA
//	GPIO_WriteBit(GPIOA,GPIO_Pin_5,0);						//输出低电平
	delay_us(15);											//延时15微秒
}
