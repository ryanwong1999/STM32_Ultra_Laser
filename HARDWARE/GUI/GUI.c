...
//			for(t=0;t<8;t++)
//		    {                 
//		        if(temp&0x01)LCD_ShowChar(x+t,y+pos);//????     
//		        temp>>=1; 
//		    }
//		}
//	}
...
void LCD_ShowChar1(u16 x,u16 y,u8 num,u8 mode)
{
  u8 temp;
  u8 pos,t;
	u16 x0=x;
	u16 colortemp=GREEN;      
    if(x>160-12||y>128-12)return;	    
	//????		   
	num=num-' ';//???????
	Address_set(x,y,x+8-1,y+12-1);      //??????
	if(mode) //?????
	{
		for(pos=0;pos<12;pos++)
		{ 
			temp=ascii_1206[(u16)num*12+pos];		 //??1608??
			for(t=0;t<8;t++)
		    {                 
		        if(temp&0x01)POINT_COLOR=colortemp;
				else POINT_COLOR=BLACK;
	...
	}else//????
	{
		for(pos=0;pos<12;pos++)
		{
			...
			for(t=0;t<8;t++)
		    {                 
		        if(temp&0x01)LCD_DrawPoint1(x+t,y+pos);//????     
		        temp>>=1; 
		    }
		}
	}
...
{         	
	...
	for(t=0;t<z_len;t++)
	{
		...
		if(enshow==0 && t<(z_len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar1(x+(size2/2)*t,y,' ',size2);
				continue;
			}
			else
			enshow=1;
		}
		LCD_ShowChar1(x+(size2/2)*t,y,temp+'0',size2); 
	}
	//???
	LCD_ShowChar1(x+(size2/2)*(z_len),y,'.',size2); 
  ...
  //????
	for(t=0;t<f_len;t++)
	{
		...
		LCD_ShowChar1(x+(size2/2)*(t+z_len)+5,y,temp+'0',size2); 
	}
}
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar1(x+8*t,y,' ',1);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar1(x+8*t,y,temp+48,1); 
	}
} 
